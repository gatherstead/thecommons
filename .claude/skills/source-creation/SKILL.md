...
description: 'Classify one or more submitted URLs as an ICS Feed, Web Scraper, or HTTP Fetch ingestion source, then build the corresponding extractor code using the existing infrastructure in backendServer/ingestion. Use when the user gives a URL or list of URLs to add as an event source, says "classify this source", "add this as an ingestion source", "is this a scraper or ICS feed", or invokes /source-creation.'
---
...
One URL, or a list of URLs (newline- or comma-separated), each optionally with a name/town
hint. If the input is empty, ask for a URL or list before proceeding.
