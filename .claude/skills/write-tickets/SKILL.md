---
name: write-tickets
description: 'Turn a feature description or an indented backlog markdown file into clear, self-contained, paste-ready engineering tickets for The Commons. Each ticket carries its own context (affected files, approach, why, acceptance criteria) so it can be dropped into a cold Claude Code instance and executed. Returns tickets in dependency order and marks which can run in parallel. Use when the user says "make tickets", "turn this backlog into tickets", "break this feature down", "write tickets for X", or invokes /write-tickets.'
---
...
This skill produces tickets. It does **not** implement them.

## Inputs

The user gives you one of:

- **A feature description** — a paragraph or two of English describing something they want built (see the "example prompt" the user may have pasted).
