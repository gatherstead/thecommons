...
from broadcast.access import AccessResult, hash_code, resolve_access
...
class CodePathTest(TestCase):
    def test_valid_code_in_budget_via_header(self):
        code = _make_code(raw="MYCODE", tier=1, max_uses=5)
...
class AccessInfoEndpointTest(TestCase):
    ...
    def test_valid_code_returns_200_with_trial_info(self):
        _make_code(raw="INFOCODE", tier=1, max_uses=3)
        resp = self.client.get(
            "/broadcast/access",
...
class PreviewMeteringTest(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.code = _make_code(raw="TRIALCODE", tier=1, max_uses=3)
