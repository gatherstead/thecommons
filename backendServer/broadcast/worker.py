...
Dispatch is on-demand: the Celery `broadcast` queue task (broadcast.tasks)
drains submissions by calling run_once(). The broadcast-worker systemd unit
runs `celery -A backend worker -Q broadcast -c 1`, mirroring the scrape
worker. run_broadcast_worker is a --once debug helper only, not the primary
entrypoint.
"""
