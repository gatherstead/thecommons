...
class AddSourceViewTests(TestCase):
    ...
    def test_idempotent_same_url_no_duplicate(self, mock_validate):
        ...
        resp = add_source(self._request({'ics_url': url, 'source_name': 'Cal', 'prompt_suffix': ''}))
        self.assertEqual(resp.status_code, 200)
        data = json.loads(resp.content)
    ...
    def test_update_fields_on_repost(self, mock_validate):
        ...
        url = 'https://example.com/events.ics'
        add_source(self._request({'ics_url': url, 'source_name': 'Old Name', 'prompt_suffix': 'old suffix'}))

        add_source(self._request({'ics_url': url, 'source_name': 'New Name', 'prompt_suffix': 'new suffix'}))
