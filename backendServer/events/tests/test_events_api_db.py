...
class EventDetailDeleteTests(TestCase):
    ...
    def test_get_one_missing_is_404(self):
        resp = self.client.get(reverse("one-event", args=["00000000-0000-0000-0000-000000000000"]))
        self.assertEqual(resp.status_code, 404)

    def test_owner_can_delete(self):
        resp = self._client_for(self.owner).delete(reverse("one-event", args=[self.event.uuid]))
        self.assertEqual(resp.status_code, 204)
        self.assertFalse(self.event.__class__.objects.filter(uuid=self.event.uuid).exists())

    def test_non_owner_cannot_delete(self):
        other = make_user("LOCAL")
        resp = self._client_for(other).delete(reverse("one-event", args=[self.event.uuid]))
