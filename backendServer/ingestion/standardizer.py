...
def _generate_json(client, prompt: str, *, log_label: str = "") -> dict:
    ...
    Extracted from `standardize_event` so any other Gemini caller can reuse the
    same fallback/retry machinery instead of standing up a second client path.
    `log_label` is folded into log lines to identify the caller (e.g. a raw
    event title) — purely cosmetic, doesn't affect behavior.

    Raises RuntimeError (chained from the last underlying error) if every model
    fails; callers needing a never-raise contract catch it themselves rather
    than this function swallowing it, so `standardize_event`'s existing
    raise-on-total-failure behavior is unchanged.
    """
...
def standardize_event(raw_event: RawEvent, prompt_suffix: str = "") -> StagedEvent:  # noqa: C901  # LLM standardization with fallbacks; complexity is inherent
    ...
    # through here, unlike the per-scraper guard (morrisvillechamber) that
    # only covers its own source. Checked against
